...
1. Agent与用户业务高度的紧密性，一旦Agent出现任何异常可能是致命的(如错误的使用了ASM的操作指令去修改类、类验证错误等)，所以对Agent自身的稳定性、兼容性要求非常高。
...
总结就是写动态链接库的方式实现起来有比较大的局限性，跨平台实现起来会比较麻烦，所以优先考虑使用ASM修改的方式。
